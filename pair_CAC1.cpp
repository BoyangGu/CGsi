   ...
   singular = LUPDecompose(mass_copy, max_nodes_per_element,0.00000000000001,  pivot);
		...
		for (ii = 0; ii < inum; ii++) {
			...
			if (current_element_type == 0) atomic_counter += 1;
				...
				if (current_element_type == 2) {                                         //wedge element                
					nodes_per_element = 6;
				}				
				if (current_element_type == 1) {                                         //cubic element
					nodes_per_element = 8;
				}
				if (current_element_type == 0) {
					nodes_per_element = 1;
...
/*  if (current_element_type == 1){
  ...
  dof_set[5][3] = 7;
   ...
   if (current_element_type == 2){
	   */
  //surface selection array            0-pos x plane 1-neg x plane 2-y plane 3-z plane 4-yz plane
  surf_set[0][0] = 1;
  surf_set[1][0] = 2;
//  surf_set[1][1] = 1;
  surf_set[2][0] = 3;
...
if(current_element_type == 2){
quadrature_node_count=6;
memory->create(quadrature_weights,quadrature_node_count,"pairCAC:quadrature_weights");
memory->create(quadrature_abcissae,quadrature_node_count,"pairCAC:quadrature_abcissae");
quadrature_weights[0]=1/6;
quadrature_weights[1]=1/6;
quadrature_weights[2]=1/6;
quadrature_weights[3]=1/6;
quadrature_weights[4]=1/6;
quadrature_weights[5]=1/6;

quadrature_abcissae[0][0]=1/6;quadrature_abcissae[0][1]=1/6;quadrature_abcissae[0][2]=-0.57735026919;
quadrature_abcissae[1][0]=2/3;quadrature_abcissae[1][1]=1/6;quadrature_abcissae[1][2]=-0.57735026919;
quadrature_abcissae[2][0]=1/6;quadrature_abcissae[2][1]=2/3;quadrature_abcissae[2][2]=-0.57735026919;
quadrature_abcissae[3][0]=1/6;quadrature_abcissae[3][1]=1/6;quadrature_abcissae[3][2]=0.57735026919;
quadrature_abcissae[4][0]=2/3;quadrature_abcissae[4][1]=1/6;quadrature_abcissae[4][2]=0.57735026919;
quadrature_abcissae[5][0]=1/6;quadrature_abcissae[5][1]=2/3;quadrature_abcissae[5][2]=0.57735026919;
memory->create(shape_quad_result, max_nodes_per_element, quadrature_node_count, "pairCAC:shape_quad_result");
memory->create(shape_quad_interior, max_nodes_per_element, quadrature_node_count, "pairCAC:shape_quad_interior");
}
...
shape_function(quadrature_abcissae[i],quadrature_abcissae[j],quadrature_abcissae[k],2,ii+1);           // 2 for cubic
            }
        }
}
}
...
for(int ii=0;ii<max_nodes_per_element;ii++){
      for (int i=0; i< quadrature_node_count;i++){
shape_quad_result[ii][i]=
shape_function(quadrature_abcissae[i][0],quadrature_abcissae[i][1],quadrature_abcissae[i][2],3,ii+1);           //3 for wedge
	  }
}
//}
...
//if(current_element_type == 2){                                  //!!
	...
	for (int i = 0; i< quadrature_node_count; i++) {
				result += quadrature_weights[i][0] * shape_quad_result[ii][i]*shape_quad_result[jj][i];
...
void PairCAC::compute_forcev(int iii){
	...
	unit_cell_mapped[1] = 1 / double(current_element_scale[1]);
	unit_cell_mapped[2] = 1 / double(current_element_scale[2]);
...
//	if(current_element_type == 2){                              //wedge element
		for (int i = 0; i < quadrature_node_count; i++) {
					...
					sq=s = interior_scale[0] * quadrature_abcissae[i][1];
					tq=t = interior_scale[1] * quadrature_abcissae[i][2];
					wq=w = interior_scale[2] * quadrature_abcissae[i][3];
					s = unit_cell_mapped[0] * (int(s / unit_cell_mapped[0]));
					t = unit_cell_mapped[1] * (int(t / unit_cell_mapped[1]));
					w = unit_cell_mapped[2] * (int(w / unit_cell_mapped[2]));
					...
					for (int js = 0; js < nodes_per_element; js++) {
						for (int jj = 0; jj < 3; jj++) {
							...
							force_column[js][jj] = force_column[js][jj] + coefficients*force_density[jj] * shape_function(sq, tq, wq, 3, js + 1);       //3 for wedge
...
//			for (int i = 0; i < isurface_counts[0]; i++) {
            ...
            for(int j=sc*3; j<sc*3+3; j++){
						...
						tq = t = interior_scale[1] * quadrature_abcissae[j][1];
						wq = w = interior_scale[2] * quadrature_abcissae[j][2];
						t = unit_cell_mapped[1] * (int(t / unit_cell_mapped[1]));
						w = unit_cell_mapped[2] * (int(w / unit_cell_mapped[2]));
						...
						double coefficients = unit_cell_mapped[0] * interior_scale[1] *
							interior_scale[2] * quadrature_weights[j];
						if (update->ntimestep == reneighbor_time||update->whichflag==2)
							quad_list_build(iii, s, t, w);
						quadrature_energy = 0;
						force_densities(iii, s, t, w, coefficients,
		...
		// t axis contributions              t=0 surface
		...
		for (int sc = 0; sc < 1; sc++) {
            ...
            for(int k=0; k<2; k++){
						...
						sq = s = interior_scale[0] * quadrature_abcissae[0][0]*sign[j];
						s = unit_cell_mapped[0] * (int(s / unit_cell_mapped[0]));
						t = 0 - i*unit_cell_mapped[1] * sign[sc];                      //!

						wq = w = interior_scale[2] * (0.211325*sign[1-k]+k);
						w = unit_cell_mapped[2] * (int(w / unit_cell_mapped[2]));
		...
		//w axis distributions              w=0 surface
		for (int sc = 0; sc < 1; sc++) {
			for (int i = 0; i < isurface_counts[0]; i++) {
            for(int j=0; j<2; j++){
            for(int k=0; k<2; k++){
						...
						sq = s = interior_scale[0] * quadrature_abcissae[0][0]*sign[j];
						s = unit_cell_mapped[0] * (int(s / unit_cell_mapped[0]));
						tq = t = interior_scale[1] * (0.211325*sign[1-k]+k);
						t = unit_cell_mapped[1] * (int(t / unit_cell_mapped[1]));
						w = 0 - i*unit_cell_mapped[2] * sign[sc];                //!
		...
		for (int sc = 1; sc < 2; sc++) {                 //oblique surface             not sure
			for (int i = 0; i < isurface_counts[0]; i++) {
            for(int j=0; j<2; j++){
            for(int k=0; k<2; k++){
						...
						s = unit_cell_mapped[0] * (int(s / unit_cell_mapped[0]));
						tq = t = interior_scale[1] * (0.211325*sign[1-k]+k);
						t = t- i*unit_cell_mapped[1] * sign[sc];
//						w = sign[sc] - i*unit_cell_mapped[2] * sign[sc];                //!
						wq = w = interior_scale[1] * (1-(0.211325*sign[1-k]+k));
						w = w - i*unit_cell_mapped[2] * sign[sc];

						double coefficients = interior_scale[2] * interior_scale[0] *
							interior_scale[1];                                                           //!
						if (update->ntimestep == reneighbor_time||update->whichflag==2)
							quad_list_build(iii, s, t, w);
						quadrature_energy = 0;
				...
				for (int j = 0; j < surface_countsy; j++) {
					for (int k = 0; k < 2; k++) {
						...
						if (sc == 0) {
							...
							sq = s = 1 - (i )*unit_cell_mapped[0];
							wq = w = 0 + (j )*unit_cell_mapped[2];
							tq = t = interior_scale[1] * (0.211325*sign[1-k]+k);
							t = unit_cell_mapped[1] * (int(t / unit_cell_mapped[1]));
						}
						else if (sc == 1) {
							sq = s = 1 - (i )*unit_cell_mapped[0];
							tq = t = interior_scale[1] * (0.211325*sign[1-k]+k);
							wq = w = interior_scale[2] * (1-(0.211325*sign[1-k]+k));
							t = unit_cell_mapped[1] * (int(t / unit_cell_mapped[1]));
							w = unit_cell_mapped[2] * (int(w / unit_cell_mapped[2]));
						}
						else if (sc == 2) {
							sq = s = 1 - (i )*unit_cell_mapped[0];
							tq = t = 0 + (j )*unit_cell_mapped[2];
							wq = w = interior_scale[2] * (0.211325*sign[1-k]+k);
							w = unit_cell_mapped[2] * (int(w / unit_cell_mapped[2]));
						}
						else if (sc == 3) {
							...
							sq = s = interior_scale[0] * (0.211325*sign[1-k]+k);
							s = unit_cell_mapped[0] * (int(s / unit_cell_mapped[0]));
							tq = t = 0 + (i )*unit_cell_mapped[1];
							wq = w = 1 - (j )*unit_cell_mapped[2];
						...
						double coefficients = unit_cell_mapped[0] * unit_cell_mapped[1] * interior_scale[0] *                  //interior_scale[?]
						...
						for (int js = 0; js < nodes_per_element; js++) {
							for (int jj = 0; jj < 3; jj++) {
								...
								force_column[js][jj] = force_column[js][jj] + coefficients*force_density[jj] *
									shape_function(sq, tq, wq, 3, js + 1);
				...
				for (int j = 0; j < isurface_counts[1]; j++) {
					for (int k = 0; k < isurface_counts[2]; k++) {
						...
						if (sc == 0) {
							...
							s = 1 - (i )*unit_cell_mapped[0];
							t = 0 + (j )*unit_cell_mapped[1];
							w = 1 - (k )*unit_cell_mapped[2];

						}
						else if (sc == 1) {
							s = 1 - (i )*unit_cell_mapped[0];
							t = 0 + (j )*unit_cell_mapped[1];
							w = 0 + (k )*unit_cell_mapped[2];
						}
						else if (sc == 2) {
							s = 1 - (i )*unit_cell_mapped[0];
							t = 1 - (j )*unit_cell_mapped[1];
							w = 0 + (k )*unit_cell_mapped[2];
						}
						else if (sc == 3) {
							s = -1 + (i )*unit_cell_mapped[0];
...
void PairCAC::quad_list_build(int iii, double s, double t, double w) {
    ...
    nodes_per_element=nodes_count_list[current_element_type];
		...
		for (int kkk = 0; kkk < nodes_per_element; kkk++) {
			shape_func = shape_function(unit_cell[0], unit_cell[1], unit_cell[2], 3, kkk + 1);
		...
		if (outer_neighflag) { rcut = 2 * cut_global_s + cutoff_skin; }                       //cut_global_s, cutoff_skin
		else { rcut = cut_global_s + cutoff_skin; }

		int origin_type = type_array[poly_counter];                       //poly_counter
		...
		double norm_a = sqrt(a[0] * a[0] + a[1] * a[1] + a[2] * a[2]);                          //norm of the vector
		double norm_b = sqrt(b[0] * b[0] + b[1] * b[1] + b[2] * b[2]);
		double norm_c = sqrt(c[0] * c[0] + c[1] * c[1] + c[2] * c[2]);

		double proj_b2a = (b[0] * a[0] + b[1] * a[1] + b[2] * a[2]) / norm_a;                    //a.b

		//compute component of b normal to a
		double b_orth[3];
		b_orth[0] = b[0] - proj_b2a*a[0] / norm_a;                                         //components of b perpendicular to a
		b_orth[1] = b[1] - proj_b2a*a[1] / norm_a;
		b_orth[2] = b[2] - proj_b2a*a[2] / norm_a;

		double proj_c2a = (c[0] * a[0] + c[1] * a[1] + c[2] * a[2]) / norm_a;              //components of c perpendicular to a           
 		double proj_c2b = (b[0] * c[0] + b[1] * c[1] + b[2] * c[2]) / norm_b;              //components of c perpendicular to b
		...
		double norm_c_orth = sqrt(c_orth[0] * c_orth[0] + c_orth[1] * c_orth[1]
			+ c_orth[2] * c_orth[2]);                                        //norm of the vector c perpendicular to ab plane
		...
		int s_upper_limit;
		int s_lower_limit;
		for (int polyscan = 0; polyscan < current_poly_count; polyscan++) {                                  //t,w      0-1? 
			scan_type = type_array[polyscan];
//			for (int wcount = -w_span; wcount < w_span + 1; wcount++) {
						for (int wcount = 0; wcount < w_span + 1; wcount++) {
//				t_lower_limit = -int((rcut + proj_c2b_orth*wcount) / norm_b_orth) - 1;
				t_lower_limit = -int((rcut + proj_c2b_orth*wcount) / norm_b_orth) ;
					...
					for (int scount = s_lower_limit; scount < s_upper_limit + 1; scount++) {
						...
						if (outofbounds == 0) {
							for (int kk = 0; kk < nodes_per_element; kk++) {
								shape_func2 = shape_function(scanning_unit_cell[0], scanning_unit_cell[1], scanning_unit_cell[2], 3, kk + 1);
...
void PairCAC::neighbor_accumulate(double x,double y,double z,int iii,int inner_neigh_initial, int outer_neigh_initial){
  ...
  firstneigh = list->firstneigh;
	...
	for (jj = 0; jj < jnum; jj++) {
		...
		if (neighbor_element_type == 2) {
			unit_cell_mapped[0] = 2 / double(neighbor_element_scale[0]);
			unit_cell_mapped[1] = 1 / double(neighbor_element_scale[1]);
			unit_cell_mapped[2] = 1 / double(neighbor_element_scale[2]);
...
//0-pos x plane 1-neg x plane 2-y plane 3-z plane 4-yz plane
...
//					shape_args[0] = 1 - unit_cell_mapped[0] / 2;
				...
				else if (surf_select[0] == 5) {               //yz plane !
				...
				for (int kk = 0; kk < neigh_nodes_per_element; kk++) {
					shape_func2 = shape_function(shape_args[0], shape_args[1], shape_args[2], 3, kk + 1);
			...
			for (int si = 0; si < 2; si++) {               //!
				...
				if (swap_min) {
					...
					for (int swap_surface = si-scan_found; swap_surface > 0; swap_surface--) {
							...
							swap_dof_set[3] = sort_surf_set[si - swap_surface][0];
							...
							sort_surf_set[si][0] = swap_dof_set[3];
		...
		//test the 6 surfaces for proximity satisfying the cutoff radius
			for (int si = 0; si < 2; si++) {                                                    //test the 6 surfaces!!
				if (complete == 0) {
					dof_surf_list[0] = sort_dof_set[si][0];
					dof_surf_list[1] = sort_dof_set[si][1];
					dof_surf_list[2] = sort_dof_set[si][2];
					...
					double unit_cell_min = unit_cell_mapped[0];                              //unit cell min
	...
	int list_nodes_per_element;
		if (etype == 2) {
			...
			for (int kk = 0; kk < list_nodes_per_element; kk++) {
				shape_func = shape_function(ucells, ucellt, ucellw, 3, kk + 1);
...
if(flag==3){                                      //wedge element

    if(index==1){
    shape_function=w*(1+s)/2;
    }
    else if(index==2){
    shape_function=(1-t-w)*(1+s)/2;
    }
    else if(index==3){
    shape_function=t*(1+s)/2;
    }
    else if(index==4){
    shape_function=w*(1-s)/2;
    }
...
)
{
	...
	*/
	...
	pz = 0;
	for (int kk = 0; kk < neigh_nodes_per_element; kk++) {
		shape_func2 = shape_function(surf_args[0], surf_args[1], surf_args[2], 3, kk + 1);
...
)
{
	...
	*/
	...
	pz2 = 0;
	for (int kk = 0; kk < neigh_nodes_per_element; kk++) {
		shape_func1 = shape_function_derivative(surf_args[0], surf_args[1], surf_args[2], 3, kk + 1, deriv_select[0]);
		shape_func2 = shape_function_derivative(surf_args[0], surf_args[1], surf_args[2], 3, kk + 1, deriv_select[1]);
